...
def convert(input_path, metadata=None, verbose=False):
    ...
    if not metadata:
        dataset_metadata = {
            ...
            "mdf_source_name": "gw100",                     # REQ string: Unique name for dataset
            "mdf-publish.publication.collection": "GW100",  # RCM string: Collection the dataset belongs to
...
#            "license": ,                             # RCM string: License to use the dataset (preferrably a link to the actual license).
            ...
            "dc.title": "Benchmark of G0W0 on 100 Molecules",                            # REQ string: Title of dataset
            "dc.creator": "The University of Chicago, Argonne National Laboratory",                          # REQ string: Owner of dataset
            "dc.identifier": "http://www.west-code.org/database/gw100/index.php",                       # REQ string: Link to dataset (dataset DOI if available)
...
#    dataset_validator = Validator(dataset_metadata, strict=False)
    ...
    for record in tqdm(parse_tab(data), desc="Processing records", disable= not verbose):
        link = "http://www.west-code.org/database/gw100/pag/" + record["cas"] + ".php"
        record_metadata = {
...
#            "license": ,                             # OPT string: License to use the record (if different from dataset) (preferrably a link to the actual license).
            ...
            "dc.title": "GW100 - " + record["name"],                            # REQ string: Title of record
...
if __name__ == "__main__":
    import paths
    convert(paths.datasets+"west_code/gw100.csv", verbose=True)
